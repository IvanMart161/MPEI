...
%     dem_sig_3 = get_dem(adc_signal, nco_sig_3, na, ng_val, nco_type_3);
%     [fdem_3, sdem_3] = get_spectrum(dem_sig_3, fs, weighting);
% 
%     figure(100);
%     plot(fdem_3/1e6, mag2db(abs(sdem_3)), 'LineWidth', 2);
%     grid on; axis tight;
%     title(sprintf('Output DEM, n_g = %d (fixed)', ng_val));
%     xlabel('f, MHz'); ylabel('dB');
%     set(gca, 'Fontsize', 28, 'Fontname', 'Times New Roman');
%     drawnow;
...
% close(100);
% 
% %% Пункт 4: исследование разбаланса квадратур (nco_type = 'single')
% disp('Моделирование и сохранение для Пункта 4...');
% nco_type_4 = 'single';
% ng_val_4 = 13; 
% 
% cases = [
%     0,   0;   % Идеальный случай
%     1,   0;   % Только амплитудный разбаланс 1 дБ
%     0,   5;   % Только фазовый разбаланс 5 град
%     1.5, 5    % Совместный разбаланс
% ];
% 
...
%     cur_pmv = cases(k, 2);
% 
%     [nco_sig_4, ~] = get_nco(ng_val_4, fg, t, nco_type_4, cur_amv, cur_pmv);
%     dem_sig_4 = get_dem(adc_signal, nco_sig_4, na, ng_val_4, nco_type_4);
%     [fdem_4, sdem_4] = get_spectrum(dem_sig_4, fs, weighting);
% 
%     figure(200);
%     plot(fdem_4/1e6, mag2db(abs(sdem_4)), 'LineWidth', 2);
%     grid on; axis tight;
%     title(sprintf('Output DEM: amv = %.1f dB, pmv = %.1f deg', cur_amv, cur_pmv));
%     xlabel('f, MHz'); ylabel('dB');
%     set(gca, 'Fontsize', 28, 'Fontname', 'Times New Roman');
%     drawnow;
...
% close(200);
% 
% disp('Все графики успешно сгенерированы и сохранены в формате PNG!');
% 
% %% Функции
...
if ~exist(dir_tiff, 'dir'), mkdir(dir_tiff); end
...
figure(1)
    ...
    drawnow;
    xlim([0 0.005]);
    save_plots('2_1', dir_png, dir_tiff, dir_fig);
...
figure(2)
    ...
    set(gca,'Fontsize',28,'Fontname','Times New Roman')
    drawnow;
    save_plots('2_2', dir_png, dir_tiff, dir_fig);
...
figure(3)
    ...
    set(gca,'Fontsize',28,'Fontname','Times New Roman')
    drawnow;
    save_plots('2_3', dir_png, dir_tiff, dir_fig);
...
figure(4)
    ...
    set(gca,'Fontsize',28,'Fontname','Times New Roman')
    drawnow;
    save_plots('2_4', dir_png, dir_tiff, dir_fig);
...
figure(5)
    ...
    set(gca,'Fontsize',28,'Fontname','Times New Roman')
    drawnow;
    save_plots('2_5', dir_png, dir_tiff, dir_fig);
...
figure(6)
    ...
    set(gca,'Fontsize',28,'Fontname','Times New Roman')
    drawnow;
    save_plots('2_6', dir_png, dir_tiff, dir_fig);
...
figure(7)
    ...
    drawnow;
    xlim([0 4000]);
    save_plots('2_7', dir_png, dir_tiff, dir_fig);
...
figure(8)
    ...
    set(gca,'Fontsize',28,'Fontname','Times New Roman')  
    drawnow;
    save_plots('2_8', dir_png, dir_tiff, dir_fig);
...
for ng_val = test_ng
    ...
    set(gca, 'Fontsize', 28, 'Fontname', 'Times New Roman');
    drawnow;
    save_plots(sprintf('3_spec_ng%d', ng_val), dir_png, dir_tiff, dir_fig);
end
close(100);
...
for k = 1:size(cases, 1)
    ...
    set(gca, 'Fontsize', 28, 'Fontname', 'Times New Roman');
    drawnow;
    save_plots(sprintf('4_spec_a%d_p%d', round(cur_amv*10), round(cur_pmv)), dir_png, dir_tiff, dir_fig);
end
close(200);

disp('Все графики успешно сгенерированы и сохранены в форматах PNG, TIFF и FIG!');
...
function save_plots(filename, dir_png, dir_tiff, dir_fig)
    % Вспомогательная функция для сохранения графика в трех форматах
    print('-dpng', '-r300', fullfile(dir_png, [filename '.png']));
    print('-dtiff', '-r300', fullfile(dir_tiff, [filename '.tiff']));
    savefig(gcf, fullfile(dir_fig, [filename '.fig']));
end